...
const GenerationPage: React.FC = ({ }) => {
  ...
  const persona: Persona = 'researcher'; // Static persona for this page
  ...
  React.useEffect(() => {
    // console.log('[Researcher Page useEffect] persona, contentType updated:', persona, contentType);
  }, [persona, contentType]);

  React.useEffect(() => {
    console.log('[Researcher Page useEffect] generatedPromptResult state updated:', generatedPromptResult);
    if (generatedPromptResult) {
      console.log('[Researcher Page useEffect] systemPrompt from state (first 200 chars):', generatedPromptResult.systemPrompt?.substring(0, 200));
      console.log('[Researcher Page useEffect] systemPrompt from state total length:', generatedPromptResult.systemPrompt?.length);
    }
  ...
  const handleDocumentUpload = async (file: File) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        setFullExtractionResult(null);
        setDocumentId('');
        // setIsProcessingDocument(false); // Handled in finally
        return; 
      }

      // Expect CognitiveKernelResult
      const cognitiveKernelResult = await response.json() as CognitiveKernelResult;
      console.log('Full Kernel Response (Researcher Page):', cognitiveKernelResult); 
      ...
      if (cognitiveKernelResult && cognitiveKernelResult.extractionResult) {
        ...
        console.error('Extraction response not in expected CognitiveKernelResult format (Researcher Page):', cognitiveKernelResult);
      ...
      console.error('Error during document upload and extraction (Researcher Page):', err);
  ...
  const handleGeneratePrompt = async () => {
    ...
    if (!fullExtractionResult || !fullExtractionResult.finalConcepts) {
        console.error('[Researcher Page] No full extraction result or final concepts available to generate prompt.');
    ...
    try {
        ...
        console.log('[Researcher Page] Received API response for generate-system-prompt:', resultData);
